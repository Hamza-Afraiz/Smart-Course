...
# Cache is a best-effort speed layer; disable it under test so a Redis instance
# that outlives the function-scoped DB resets can't serve stale cross-test data.
# test_cache.py re-enables it explicitly to exercise the cache path.
...
_TEST_POSTGRES_HOST = os.getenv("TEST_POSTGRES_HOST", "localhost")
TEST_DB_NAME = "smartcourse_test"
TEST_DATABASE_URL = (
    f"postgresql+psycopg://smartcourse:smartcourse@{_TEST_POSTGRES_HOST}:5432/{TEST_DB_NAME}"
)
ADMIN_DATABASE_URL = (
    f"postgresql://smartcourse:smartcourse@{_TEST_POSTGRES_HOST}:5432/postgres"
)
...
def _ensure_test_database():
    ...
    try:
        ...
        # pgvector is required for lesson_chunks.embedding (Week 4+)
        test_conn = psycopg.connect(
            f"postgresql://smartcourse:smartcourse@{_TEST_POSTGRES_HOST}:5432/{TEST_DB_NAME}",
            autocommit=True,
        )
        try:
...
async def admin_headers(client) -> dict[str, str]:
    return await _register_and_login(client, "admin@test.com", "secret123", "admin")


async def seed_lesson_chunks(
    engine,
    *,
    course_id: uuid.UUID,
    lesson_id: uuid.UUID,
    ...
    """Insert indexed chunks for tests — avoids running the embedding model."""
