...
async def test_published_course_visible_in_list(
    client, instructor_headers, student_headers, make_published_course
):
    await make_published_course(title="To publish")
    resp = await client.get("/api/v1/courses", headers=student_headers)
...
async def test_cannot_patch_status_to_published_returns_409(client, instructor_headers):
    course = await _create_course(client, instructor_headers)
